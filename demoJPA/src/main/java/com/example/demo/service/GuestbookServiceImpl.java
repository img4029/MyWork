...
public class GuestbookServiceImpl implements GuestbookService{
	...
	// => JPA Sql 처리를 위해 정의
	//    생성자 주입 : ( @RequiredArgsConstructor를 통해 주입받음 )
	
	// GuestbookServiceImpl(GuestbookRepository repository){
	...
	public PageResultDTO<GuestbookDTO, Guestbook> pageList(PageRequestDTO requestDTO) {
		...
		// => Service에 정의한 entityToDto 메서드를 이용하여 entity를 Dto로 변환
		
		
		// 람다식을 인자로 직접사용해도 됨 (이로서 Java도 함수형 언어라 할 수 있게 되었다고함)
		// return new PageResultDTO<GuestbookDTO, Guestbook>(result, fn);
		return new PageResultDTO<GuestbookDTO, Guestbook>(result, entity -> entityToDto(entity));
	} //pageList
